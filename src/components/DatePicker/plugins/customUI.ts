...
export interface ElementsType {
  ...
  yearMenu?: HTMLDivElement;
  monthMenu?: HTMLDivElement;
}
...
const customUI =
  ...
  (fp: CustomUIProps): CustomUIType | {} => {
    ...
    const updateDropdownMenu = () => {
      ...
      const monthItems =
        monthMenu?.querySelectorAll('.dropdown-month-item') || [];
    ...
    const renderDropdown = () => {
      const { l10n } = fp;
      const dropdown: HTMLSpanElement = document.createElement('div');
      ...
      getYears().forEach((year) => {
        const item: HTMLSpanElement = document.createElement('span');
      ...
      months.forEach((month: string, index: string) => {
